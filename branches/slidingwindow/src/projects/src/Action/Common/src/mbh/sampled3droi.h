...
#ifndef _MBH_SAMPLED_3D_ROI_H_
#define _MBH_SAMPLED_3D_ROI_H_
...
struct roi3d {
	...
	cv::Point3i tlp;  //top left 3d point(x,y,t)
	cv::Point3i whl;  //roi size, width, height and long

	roi3d(const cv::Point3i &p1, const cv::Point3i &p2)
	{
		tlp = p1;
		whl = p2;
	}
	...
	roi3d() :tlp(cv::Point3i(0, 0, 0)), whl(cv::Point3i(0, 0, 0))
	{}
};

class sampled3Droi {

	cv::Point3i _vSize;  //video size
	cv::Point3f _strideRatio; //stride overlap ratio per 3d window slide, default is 50% overlap.  0 means no overlap (stride = patchSz)

	float _sigma, _tao; //spatial and temporal scale

	cv::Point3i _stSz;  //start (smallest) 3D patch size. default is 18x18x10
	//Point3i _endSz;   //end (largest) 3D patch size. default is 192x192x14 
	cv::Point3i _scales;  //number of total scales. default is (8,8,2), 8 spatial and 2 temporal scales
	cv::Point2f _scaleStep;  //scale factor for consecutive scales, default 2^(1/2)

	//float _score, _scale;
	std::vector<roi3d> _resultRoi;
...
public:
	...
	sampled3Droi(const cv::Point3f& sRa = cv::Point3f(0.5, 0.5, 0.5),
		const cv::Point3i& stSz = cv::Point3i(18, 18, 10),
		const cv::Point3i& scls = cv::Point3i(8, 8, 2),
		const float	  sigma = 2,
		const float	  tao = 2) :
		_stSz(stSz),
		_scales(scls),
	...
	sampled3Droi(const cv::Point3i& vSz,
		const cv::Point3f& sRa = cv::Point3f(0.5, 0.5, 0.5),
		const cv::Point3i& stSz = cv::Point3i(18, 18, 10),
		const cv::Point3i& scls = cv::Point3i(8, 8, 2),
		const float	  sigma = 2,
		const float	  tao = 2) :
	...
	void resetPara(const cv::Point3i& vSz,
		const cv::Point3f& sRa,
		const cv::Point3i& stSz,
	...
	inline void resetVideoSz(const cv::Point3i& vSz)
	{
		get3Droi(vSz);
	}

	inline void setParaStrdRt(const cv::Point3f& strSz)
	{
	...
	inline void setParaScale(const cv::Point3i& scale)
	{
		if (_scales != scale)
		{
			_resultRoi.clear();
	...
	inline void setParaStSz(const cv::Point3i& stSz)
	{
		if (_stSz != stSz)
		{
			_resultRoi.clear();
	...
	void operator()(const cv::Point3i& vSz)
	{
		get3Droi(vSz);
	}
	...
	inline cv::Point3i startSz() const { return _stSz; }
	inline cv::Point3f strideRt() const { return _strideRatio; }
	inline cv::Point3i scaleSz() const { return _scales; }
	inline cv::Point2f scaleStep() const { return _scaleStep; }


	void get3Droi(const cv::Point3i& videoSz)
	{
	...
	void getPatchSz(std::vector<cv::Point3i>& patches)
	{
		...
		if (_sigma == 2 && _tao == 2)
		{
			...
			for (int t = 0; t < T0; t++)
			{
				...
				for (x0 = xy0, x1 = xy1, xy = 0; xy < tmpScls.x; xy++)
				{
					...
					{
						x1 *= xys;
						tmp = cv::Point3i(x1, x1, tt);
					}
					else
					{
						x0 *= xys;
						tmp = cv::Point3i(x0, x0, tt);
					}
	...
	bool doSampling(const cv::Point3i& patchSz)
	{
		cv::Point3i extent = _vSize - patchSz;
		if ((extent.x < 0) || (extent.y < 0) || (extent.z < 0))
			return false;

		cv::Point3i strideSz = cv::Point3i((int)((1 - _strideRatio.x)*patchSz.x), (int)((1 - _strideRatio.y)*patchSz.y), (int)((1 - _strideRatio.z)*patchSz.z));
		...
		for (int t0 = 0, t1 = 0; t0 <= extent.z; t0 += strideSz.z) {
			for (int y0 = 0, y1 = 0; y0 <= extent.y; y0 += strideSz.y) {
				for (int x0 = 0, x1 = 0; x0 <= extent.x; x0 += strideSz.x) {
					_resultRoi.push_back(roi3d(cv::Point3i(x0, y0, t0), patchSz));
...
protected:
	...
	inline std::vector<roi3d>::iterator begin() { return _resultRoi.begin(); }
	inline std::vector<roi3d>::iterator end() { return _resultRoi.end(); }

	//this method is not safe. it can change class members. use with care
	inline std::vector<roi3d>* to3Droi()
	{
		return &_resultRoi;
	}
