...
#ifndef _MBH_ST_DESCRIPTOR_H_
#define _MBH_ST_DESCRIPTOR_H_
...
struct MBHparam{
	...
	cv::Point3i				numCell;  //number of cells per descriptor, default is 4x4x3
	cv::Point3i				numBlock;  //number of subblocks per cell. default is 3x3x3
	...
	cv::Point3f				rt2ps;	 //ratio for root video size to part video size. default is (0.5, 0.5, 0.5) {_ivWrt/_ivWps = _ivHrt/_ivWps = _ivBrt/_ivBps = 0.5)
	cv::Point3i				numParts;  //number of parts per root window, default is 2x2x2
	cv::Point3f				olRatio; //overlap ratio per 3d parts, default is 50% overlap. if zero, there is no overlap for different parts. if 1, only one part for one root wind

	//sampled3Droi parameters:
	cv::Point3f				strideRatio; //stride overlap ratio per 3d window slide, default is 50% overlap
	cv::Point3i				stSz;  //start (smallest) 3D patch size. default is 18x18x10
	//Point3i _endSz;   //end (largest) 3D patch size. default is 192x192x14 
	cv::Point3i				scales;  //number of total scales. default is (8,8,2), 8 spatial and 2 temporal scales
	...
	MBHparam(
		const cv::Point3i&  numBloc_ = cv::Point3i(3, 3, 3),
		const cv::Point3i&  numCell_ = cv::Point3i(2, 2, 3),
		bool roundTp_ = 0,
		bool normBlk_ = 1,
		float cutZ_ = 0.25,
		float reSzV_ = 1.0,
		...
		const cv::Point3i& stSz_ = cv::Point3i(12, 12, 10),
		const cv::Point3i& scls = cv::Point3i(8, 8, 2),
		const cv::Point3f& r2p = cv::Point3f(0.5, 0.5, 1),
		const cv::Point3i& noPts = cv::Point3i(2, 2, 2),
		const cv::Point3f& ovlp = cv::Point3f(0.5, 0.5, 0.5),
		float	  sigma = 2,
		float	  tao = 2);
...
class stDetector {
	...
	cv::Mat				  _ft;    //save the computed feature  
	...
	void randomSampling(unsigned int nSmpls, cv::RNG &seed);  //perform random sampling with number of sameles = nSmpls
	void denseSampling();
...
public:
	...
	bool preProcessing(const std::string& fName, int fms);
	bool re_Processing(const std::string& fName, int fms, int num);

	bool preProcessing(const std::string& fName, int st, int end);

	void getRandomFeatures(cv::Mat &feature, unsigned int nSmpls, cv::RNG &seed);
	void getRandomFeaturesByRatio(cv::Mat &feature, cv::RNG &seed, float ratio = 0.1, bool lastRedo = 0);
	void getDenseFeatures(cv::Mat &feature);
	void clear();  //for memory efficient

	//bool getFeatures(Mat &feature, const string& fName, unsigned int nSmpls = 0, bool flipIm=0);

	inline void setSamplingPara(const cv::Point3i& vSz,
	...
	inline void setSamplingStrideSz(const cv::Point3f& strSz)
	{
		_s3Droi->setParaStrdRt(strSz);
	}
	inline void setSamplingScale(const cv::Point3i& strSz)
	{
		_s3Droi->setParaScale(strSz);
	}
	inline void setSamplingStSz(const cv::Point3i& strSz)
	{
		_s3Droi->setParaStSz(strSz);
	}

	inline cv::Size featureSize() const
	{
