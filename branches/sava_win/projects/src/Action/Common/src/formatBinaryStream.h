...
DEF_TYPE_ID(long long,          10);
DEF_TYPE_ID(unsigned long long, 11);
...
public:
	...
	BinClusterInStream(const std::string& filename_): parent(filename_)
	{
		...
		switch(dataTp) {
			...
			case 10:
				...
				cvTp = cv::DataType<int>::type;
				rawDataStep = _dimFeature*sizeof(long long);
				break;
			case 11:
	...
	virtual void read(cv::Mat& mat, long selectIn = -1) //if selectIn <= 0, input all data into mat, otherwise randomly select 'selectIn' rows from total '_numFeature' 
	{
		...
		else 
		{
			...
			mat = cv::Mat(selectIn, _dimFeature, cvTp);
...
public:
	...
	virtual void read(std::vector<cv::Mat>& mat, int stframes = -1, int endframes = -1) //if endframes <= 0, input all data into mat 
	{
		//std::cout<<_numFeature<<" ";
		parent::_stream.seekg(_initpos);
		cv::Mat tmp;
		...
		if (endframes <= 0 || stframes >= _frameFeature || stframes >= endframes) 
		{
			...
			{
				tmp = cv::Mat(_numFeature, _dimFeature, cvTp);
				for (int i = 0; i < _numFeature; i++)
		...
		else 
		{
			...
			if(stframes > 0) 
			{
				...
				parent::_stream.seekg(offset, std::ios_base::cur);
			}
	...
	cv::Point3i getVsz()
	{
		return cv::Point3i(_dimFeature,_numFeature,_frameFeature);
	}
...
public:
		...
		parent(filename_), _numFeature(numFeature_), _dimFeature(dimFeature_)
	{
		...
		parent::_stream<<type;
		_initpos = (long)parent::_stream.tellp();
