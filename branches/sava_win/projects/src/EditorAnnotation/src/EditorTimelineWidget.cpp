	...
	: QWidget(parent)
{
	...
	m_SequenceManager = EditorSequencesManager::getPointer();
...
void EditorTimelineWidget::onMessage(EditorMessageType i_Message, int i_ExtraParam /*= -1*/)
{
	switch (i_Message)
	{
		...
		case MESSAGE_SEQUENCE_LOADED:
			if (i_ExtraParam != 0) {
				m_Source->setFrameCount(EditorSequencesManager::getInstance().getSequence()->getNumFrames());
...
/*--------------------------------------------------------------------*/
void EditorTimelineWidget::skipForward()
{
	if (m_Source->getCurrentFrame() >= m_Source->getFrameCount() - 1)
		m_SequenceManager->loadNext(-1); //set last frame of loaded sequence
	else
		m_Source->skipForward();
}

/*--------------------------------------------------------------------*/
void EditorTimelineWidget::skipBackward()
{
	if (m_Source->getCurrentFrame() == 0)
		m_SequenceManager->loadPrev(0); //set first frame of loaded sequence
	else
		m_Source->skipBackward();
}
