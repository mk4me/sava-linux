...
#ifndef _MBH_3D_DESC_FEATUREOF_H
#define _MBH_3D_DESC_FEATUREOF_H
...
class descFeature
{
	cv::Point3i				_numCell;  //number of cells per descriptor, default is 2x2x3
	cv::Point3i				_numBlock;  //number of subblocks per cell. default is 3x3x3
	cv::Point3i				_pSz;  // minimal patch size(_numCell.x * _numBlock.x, _numCell.y * _numBlock.y, _numCell.z * _numBlock.z)
	...
	cv::Point3i				_numParts;  //number of parts per root window, default is 2x2x2
	cv::Point3f				_olRatio; //overlap ratio per 3d parts, default is 50% overlap. if zero, there is no overlap for different parts. if 1, only one part for one root window
	...
	void l2Norm(cv::Mat arr, cv::Mat& dst, float cutVal, bool L2Hys = 0) const;  //if  L2Hys is ture, then renormalize with cutVal
...
public:
	...
	descFeature(
		...
		const cv::Point3f&  rt2ps_ = cv::Point3f(0.5, 0.5, 0.5),
		const cv::Point3f&  olRto_ = cv::Point3f(0.5, 0.5, 0.5),
		const cv::Point3i&  numParts_ = cv::Point3i(2, 2, 2),
		bool roundTp_ = false,
		bool normBlk_ = true,
		float cutZ_ = 0.25,
		float reSzV_ = 1.0,
	...
	bool descFeature::preProcessor(const std::shared_ptr<sequence::IStreamedVideo>& video, int stFrame = 0, int endFrame = 0);

	//compute feature of an ST patch with top left 3d point of tlp0 and roi size  of whl0
	void computeFeature(const cv::Point3i& tlp0, const cv::Point3i& whl0, cv::Mat& outArr) const;
	void computeFeature(const cv::Point3i& tlp0, const cv::Point3i& whl0, float *const outArr) const;

	void computeFt(const cv::Point3i& tlp, const cv::Point3i& whl, float *ft, int sz, bool root = 1)const;
	...
	inline bool videoSz(cv::Point3i& vSz) const
	{
		if (_iv)
		{
			vSz = _iv->toVideoSz();
	...
	inline cv::Point3i videoSz() const
	{
		if (_iv)
		{
...
#endif//_MBH_3D_DESC_FEATUREOF_H
