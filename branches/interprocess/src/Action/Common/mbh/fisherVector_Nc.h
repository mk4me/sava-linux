#ifndef _MBH_FISHER_VECTOR_ENCODING_H_
#define _MBH_FISHER_VECTOR_ENCODING_H_
...
const float _gmmSigmaLowerBound = 1e-5f;
...
public:
	...
	fvEncoding(const std::string *iFile, int channels, const int* dim = NULL, int numCluster = 256, int mTp = 2, std::string* gmmSaveName = NULL) :_channels(channels)
	{
	...
	void getFeatures(cv::Mat hog3dFt0)
	{
		cv::Mat *buffer0 = new cv::Mat[_channels];
		for (int ch = 0; ch < _channels; ch++)
			buffer0[ch] = cv::Mat(hog3dFt0.rows, _dimension[ch], hog3dFt0.type());
		int col0 = 0;
		for (int ch = 0; ch < _channels; ch++)
		{
	...
	void getFeatures(const cv::Mat &hog3dFt, float *arr)
	{
		getFeatures(hog3dFt);
...
#endif //_MBH_FISHER_VECTOR_ENCODING_H_
