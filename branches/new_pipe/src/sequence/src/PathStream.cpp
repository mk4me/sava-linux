...
namespace sequence
{
	...
	void PathStream::getLastestPaths(std::map<Id, Path>& outPaths) const
	{
		std::map<Id, Path> paths;
		auto count = m_Frames.size();
		auto& frame = m_Frames[count - 1];
		for (auto& p : frame)
		{
			Path& path = paths[p.id];
			path.points.insert(std::make_pair(count, p.point));
			path.id = p.id;
		}
