...
void EditorInterestingPeriodWidget::onMessage(EditorMessageType i_Message, int i_ExtraParam /*= -1*/)
{
	...
	case MESSAGE_NEW_SEQUENCE_LOAD_FINISH:
		int frames = EditorSequencesManager::getInstance().getSequence()->getVideo()->getNumFrames();
		m_Scene->setSceneRect(0, 0, frames - 1, SCENE_HEIGH);
...
{
	for each (RectItem* item in m_RectsItems)
