...
int main(int argc, char *argv[])
{
	if (argc < 2)
	{
		std::cout << "Usage: getTrainTestData.exe <path>\n<path> - hmdb51 database path";
		discoverUO::wait(5);
		exit(-1);
	}
	...
	for (int i = 0; i < _channels; i++)
		fName[i] = outDir + (std::string)"gmmResults" + std::to_string(i) + (std::string)".yml";   //pre-computed GMMs
	...
	{
		std::cout << "The input precomputed pca file is wrong!" << std::endl;
		discoverUO::wait(5);
		exit(-1);
	}
	...
	for (int i = 0; i < clsNum; i++)
	{
		...
		classDir = databaseDir + std::to_string(i) + '/';
		std::cout << "Now doing folder: " << classDir << std::endl;

		
		if (!boost::filesystem::exists(classDir + "train/"))
		{
			std::cout << "Class directory does not exist.\n";
			discoverUO::wait(5);
			exit(-1);
		}
		std::vector<std::string> fileList = utils::Filesystem::getFileList(classDir + "train/");
		if (!boost::filesystem::exists(classDir + "test/"))
		{
			std::cout << "Class directory does not exist.\n";
			discoverUO::wait(5);
			exit(-1);
		}
		std::vector<std::string> testFiles = utils::Filesystem::getFileList(classDir + "test/");
...
bool loadPCA(PCA* pca0, const std::string& preComputedFl)
{
	...
	if (!fs.isOpened())
	{
		std::cout << "Can't open pca precomputed file: " << preComputedFl << std::endl;
		return false;
	}
	...
	if (chnl != _channels)
	{
		std::cout << "Wong! The precomputed pca file has different number of channels!" << std::endl;
		return false;
	}
