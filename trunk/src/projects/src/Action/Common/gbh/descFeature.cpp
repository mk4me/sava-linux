...
void opcvL1Norm(float *arr, int sz, bool useThres = 0) 
{
	...
	cv::normalize(m2, m2, 1.0, 0., cv::NORM_L1);
	m2.copyTo(m1);
}
...
void opcvL1NormSqrt(float *arr, int sz) 
{
	...
	cv::normalize(m2, m2, 1.0, 0.0, cv::NORM_L1);
	cv::sqrt(m2, m2);
...
void descFeature::l2Norm(cv::Mat arr, cv::Mat& dst, float cutVal, bool L2Hys) const 
{
	dst = arr.clone();
	float *pt;
...
descFeature::descFeature(
			...
			const cv::Point3f&  rt2ps_,
			const cv::Point3f&  olRto_,
			const cv::Point3i&  numParts_,
			bool roundTp_,
			bool normBlk_,
			float cutZ_,
			float reSzV_,
		...
		_2chnls(chnl2)
{
	...
	_pSz = cv::Point3i(_numCell.x * _numBlock.x, _numCell.y * _numBlock.y, _numCell.z * _numBlock.z);
	_dim = _iv->dimSz();  
...
bool descFeature::preProcessor(const std::shared_ptr<sequence::IStreamedVideo>& video, int stFrame, int endFrame)
{
	if (_iv)
		delete _iv;
	_iv = new IntegralVideo(video, _nbins, _fullOri,_rt2ps, _reSzVideo,_2chnls, stFrame, endFrame);
	return _iv->hasIv();
}

void descFeature::computeFeature(const cv::Point3i& tlp0, const cv::Point3i& whl0, cv::Mat& row0) const
{
	float *ft = row0.ptr<float>(0);
	computeFeature(tlp0, whl0, ft);
}

void descFeature::computeFeature(const cv::Point3i& tlp0, const cv::Point3i& whl0, float *const outArr) const
{
	float *ft = outArr;
...
//compute features for parts' windows corresponding to root's window (tlp0, whl0)
	...
	cv::Point3i tlpP0 = cv::Point3i(cvFloor(tlp0.x/_rt2ps.x), cvFloor(tlp0.y/_rt2ps.y), cvFloor(tlp0.z/_rt2ps.z));  
	//size of all parts' window corresponding to root's window
	cv::Point3i whlP0 = cv::Point3i(cvFloor(whl0.x/_rt2ps.x), cvFloor(whl0.y/_rt2ps.y), cvFloor(whl0.z/_rt2ps.z));  
	...
	//overlap size
	cv::Point3i whlOlp = cv::Point3i((int)(whlPsz.x*_olRatio.x), (int)(whlPsz.y*_olRatio.y), (int)(whlPsz.z*_olRatio.z));

	for (int iz = 0; iz < _numParts.z; iz++)
		for (int iy = 0; iy < _numParts.y; iy++)
			for (int ix = 0; ix < _numParts.x; ix++)
			{
				cv::Point3i tlpTmp = cv::Point3i(tlpP0.x + ix*(whlPsz.x-whlOlp.x), 
...
void descFeature::computeFt(const cv::Point3i& tlp, const cv::Point3i& whl, float *ft, int sz, bool root) const 
{
	cv::Point3i tlp0, whl0, blkSz, cellSz, cSz, tlpCell, tlpBlk;
	std::vector<float> desc(_dim);
					...
					for (int bz = ((tmp1.z < 0) ? (cz + tBz) : cz), olpBz = tmp1.z; bz < cz + cellSz.z -tBz;  olpBz--) {
						for (int by = ((tmp1.y < 0) ? (cy + tBy) : cy), olpBy = tmp1.y; by < cy + cellSz.y - tBy;  olpBy--) {
							for (int bx = ((tmp1.x < 0) ? (cx + tBx) : cx), olpBx = tmp1.x; bx < cx + cellSz.x - tBx; olpBx--)
							{
								if (root)
								{
									_iv->getRtDesc_u(cv::Point3i(bx, by, bz), blkSz, desc, _normBlk);
									for (int i = 0; i < _dim; i++)
										(ft + step)[i] += desc[i];
									if(_2chnls)
									{
										_iv->getRtDesc_v(cv::Point3i(bx, by, bz), blkSz, desc, _normBlk);
										for (int i = 0; i < _dim; i++)
											(ft + step+ _feaSz/2)[i] += desc[i];
									}
								}
								...
								{
									_iv->getPsDesc_u(cv::Point3i(bx, by, bz), blkSz, desc, _normBlk);
									for (int i = 0; i < _dim; i++)
										(ft + step)[i] += desc[i];
									if(_2chnls)
									{
										_iv->getPsDesc_v(cv::Point3i(bx, by, bz), blkSz, desc, _normBlk);
										for (int i = 0; i < _dim; i++)
											(ft + step+ _feaSz/2)[i] += desc[i];
									}
								}
